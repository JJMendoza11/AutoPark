...
static uint16_t u16ModVal=0;
	...
	//---PWM Initialization---
		...
		TPM0->SC|=TPM_SC_PS(1);													//Division of CLK
		TPM0->SC|=TPM_SC_CMOD(1);												//CLK
		TPM0->SC|=TPM_SC_CPWMS(0);												//Up counting
		TPM0->MOD = 800;														//Period of 20ms
		TPM0->CONTROLS[3].CnSC |= TPM_CnSC_ELSB_MASK| TPM_CnSC_ELSA_MASK | TPM_CnSC_CHIE_MASK;			//HIGH TRUE
